...
<form class="mb-2" action="" method="post">
    <div class="form-row">
        <div class="col-2">
            <h1 class="h2 text-gray-800 font-weight-bold cap animated bounceInLeft faster">Data Mapel</h1>
        ...
        <div class="col"><?php
                            if ($level == 'Admin') { ?>
                <a class="btn btn-warning animated bounceInRight faster" href="?page=tambahm" role="button"><i class="fas fa-plus"></i><span class="font-weight-bold cap">Tambah</span>
                </a><?php
                            } ?>
        </div>
    </div>
    ...
    <div class="card-body bg-dark">
        <table id="example" class="table table-striped table-dark mt-3">
            ...
            <tbody>
                ...
                if (isset($cari)) {
                    // perintah query untuk menampilkan jumlah data jurusan dari database berdasarkan kode Jurusan atau nama kelas yang sesuai dengan kata kunci pencarian
                    $query_jumlah = mysqli_query($db, "SELECT count(kode_mapel) as jumlah FROM tbl_matapelajaran
WHERE kode_mapel LIKE '%$cari%' OR nama_mapel LIKE '%$cari%'")
                ...
                // jika dilakukan pencarian data
                if (isset($cari)) {
                    // perintah query untuk menampilkan data jurusan dari database berdasarkan kode_mapel atau nama yang sesuai dengan kata kunci pencarian
...
ON tbl_matapelajaran.kode_jurusan = tbl_jurusan.kode_jurusan WHERE tbl_matapelajaran.kode_mapel LIKE '%$cari%' OR nama_mapel LIKE '%$cari%'
ORDER BY kode_mapel DESC LIMIT $mulai, $batas")
                ...
                // jika tidak dilakukan pencarian data
                    ...
                    // data yang ditampilkan mulai = $mulai sampai dengan batas = $batas
                    $query = mysqli_query($db, "SELECT * FROM tbl_matapelajaran INNER JOIN tbl_jurusan 
ON tbl_matapelajaran.kode_jurusan = tbl_jurusan.kode_jurusan  WHERE tbl_matapelajaran.kode_mapel ORDER BY kode_mapel DESC LIMIT $mulai, $batas")
                        or die('Ada kesalahan pada query jurusan: ' . mysqli_error($db));
                }
                // tampilkan data
                while ($data = mysqli_fetch_assoc($query)) { ?>
                    <tr class="cap centr">
                        ...
                        if ($level == 'Admin') { ?>
                            <td>
                                <a title="Ubah" class="btn btn-success" href="?page=ubahm&kode_mapel=<?php echo $data['kode_mapel']; ?>"><i class="fas fa-edit"></i></a>
                                <a title="Hapus" class="btn btn-danger" href="mapel_proses_hapus.php?kode_mapel=<?php echo $data['kode_mapel']; ?>" onclick="return confirm('Anda yakin ingin menghapus jurusan <?php echo $data['nama_mapel']; ?>?');"><i class="fas fa-trash"></i></a>
                            </td>
                        <?php
                        } ?>
                    </tr>
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                ...
                // jika halaman aktif <= jumlah halaman, maka button Selanjutnya = aktif
                    ...
                    <li class="page-item"><a class="page-link text-warning bg-dark" href="?hal=<?php echo $page + 1 ?>">Selanjutnya</a></li>
                <?php } ?>
            </ul>
        </nav>
    </div>
